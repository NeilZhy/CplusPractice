...
private:
	...
	//拷贝构造函数是我需要非常认真对待的一个函数
	Node* CBinary(Node* root)   //拷贝构造函数
	{
		Node* t = NULL;
		...
		t = new Node(root->_data);
		t->_left = CBinary(root->_left);
		t->_right = CBinary(root->_right);
		return t;
	}
